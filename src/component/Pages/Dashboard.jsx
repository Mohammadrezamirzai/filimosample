        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 p-8 flex flex-col items-center space-y-6">
          ...
          {user ? (
            <div className="w-full bg-white/10 rounded-xl p-4 flex flex-col items-center space-y-2">
              <div className="text-white font-semibold">
                نام: <span className="text-green-400">{user.firstName}</span>
              </div>
              <div className="text-white font-semibold">
                نام خانوادگی:{" "}
                <span className="text-green-400">{user.lastName}</span>
              </div>
              <div className="text-white font-semibold">
                ایمیل: <span className="text-green-400">{user.email}</span>
              </div>
              <div className="text-white font-semibold">
                شماره تلفن: <span className="text-green-400">{user.phone}</span>
              </div>
            </div>
          ) : (
            <div className="text-orange-400 font-bold text-lg mt-4">
          ...
          <div className="w-full flex flex-col md:flex-row gap-4 mt-6">
            <button
              ...
              className="flex-1 py-3 px-4 cursor-pointer rounded-xl  bg-gradient-to-r from-green-500 to-blue-500 text-white font-bold shadow hover:from-green-600 hover:to-blue-600 transition-all">
              برو به خانه
            </button>
            <button
              className="flex-1 py-3 px-4 cursor-pointer  rounded-xl bg-gradient-to-r from-orange-500 to-pink-500 text-white font-bold shadow hover:from-orange-600 hover:to-pink-600 transition-all"
              onClick={() => {
