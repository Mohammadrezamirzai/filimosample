...
import { getSearchValue } from "../utils/api/api";
...
export default function Results() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#1a1a2e] to-[#16213e] mt-16 py-12 px-4 flex flex-col items-center">
      ...
      {loading && <div className="min-h-screen mt-24 text-orange-500 text-2xl font-bold text-center">در حال بارگذاری...</div>}
      ...
      {!loading && !error && (
        <div className="w-full max-w-7xl rounded-2xl bg-white/5 backdrop-blur-md shadow-xl p-6 md:p-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {movies.length === 0 ? (
            <div className="col-span-full text-center text-gray-300">نتیجه‌ای یافت نشد.</div>
          ) : (
            movies.map((movie, idx) => (
              <div key={idx} className="flex justify-center">
                <Card
                  ...
                  title={movie.title}
                  imdbRating={movie.imdb_rating || movie.imdbRating || movie.rating}
                  year={movie.year || (movie.released && movie.released.slice(0, 4))}
                  genre={movie.genre || (Array.isArray(movie.genres) ? movie.genres.join(", ") : undefined)}
